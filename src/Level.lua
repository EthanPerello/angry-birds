...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        local xVel2, yVel2 = 0, 0
        if self.split == true then
            xVel2, yVel2 = self.aliens[#self.aliens-1].body:getLinearVelocity()
        end

        local xVel3, yVel3 = 0, 0
        if self.split == true then
            xVel3, yVel3 = self.aliens[#self.aliens].body:getLinearVelocity()
        end
        
        -- if we fired our alien to the left or it's almost done rolling, respawn
        if (xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5))
        and (math.abs(xVel2) + math.abs(yVel3) < 1.5)
        and (math.abs(xVel3) + math.abs(yVel3) < 1.5)then
            self.launchMarker.alien.body:destroy()
            self.collided = false
            if self.split == true then
                self.aliens[#self.aliens].body:destroy()
                self.aliens[#self.aliens-1].body:destroy()
                table.remove(self.aliens)
                table.remove(self.aliens)
                self.split = false
            end
            self.launchMarker = AlienLaunchMarker(self.world)
        ...
        -- if we press space and the alien has not collided with anything, have the alien split into 3
        if love.keyboard.wasPressed('space') and self.collided == false then
            gSounds['split']:play()

            local alien1 = Alien(self.world, 'round', xPos, yPos+10, 'Player')
            alien1.body:setLinearVelocity(xVel, yVel)
            alien1.fixture:setRestitution(0.4)
            alien1.body:setAngularDamping(1)

            local alien2 = Alien(self.world, 'round', xPos, yPos-10, 'Player')
            alien2.body:setLinearVelocity(xVel, yVel)
